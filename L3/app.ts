 ...
 */
const sum2 = combine(10,50, "as-number");// 60 aayega
const combinedName = combine("patel", "mernStack", "as-string")// patelmernStack aayegaa answer
...
/**so LITTERAL TYPE mein hum core type nhi likhte hai jaise int, string, boolean etc  hum apna
 * khud ka Type bana lete hai "as-number" "as-string" toh aise khud ka Types bana sakte hai
 * so hum aise kuch bhi ttype bana sakte hai jaise "as-aman"
  */


//Now dekhenge TYPE ALIAS/custom Types
...
//so first of all why we have to use type alias in object
// const user : {
//     name : string;
//     age : number;
// } = {
//     name : "aman",
//     age : 23
// } here you can see that its not Readable 
//so without type alias its very difficult to Read object that's why we have to use type alias
// type alias in object
type User = {
    name : string;
    age : number;
...
const user : User = {//toh type alias se object  jyada Readable ban jata hai so here we use User type
    name: "patel ",
    age : 34,
    skills : ["React","Node"]
}
...
function greet(user : User){//so idhar user object pass kar rghe hai toh iss user ka type i.e User denge nott a string etc
